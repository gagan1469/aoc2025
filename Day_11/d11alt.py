...
def get_count_of_paths_between_nodes(grph: nx.DiGraph, start_node: str, end_node: str) -> int:
    results = []
    exits, results = discover_successors(grph=grph, node_name=start_node, end_node=end_node)
    print(f'There are {exits} paths from {start_node} to {end_node}')

    return exits

def build_directed_graph(data_file: str, name: str = 'directed_graph'):
    all_edges = load_data_set(data_file=data_file)
...
def main():
    ...
    parser.add_argument('--start', type=str, required=False, default=K_START_NODE)
    parser.add_argument('--end', type=str, required=False, default=K_END_NODE)
    parser.add_argument('--file', type=str, required=False, default=r'Day_11\inputs.txt')
    ...
    start_time = datetime.now().isoformat(timespec='seconds')
    print(f'{start_time}::Starting counting all paths from {start_node} to {end_node}')
    print(f'Graph defined in {file_name}')
    ...
    if execute_part_1:
        ...
        part1_solution(file_name, start_node)
        input('Waiting to start Part 2...')
    ...
    grph = build_directed_graph(file_name)
    count = get_count_of_paths_between_nodes(grph, start_node, end_node)

    end_time = datetime.now().isoformat(timespec='seconds')
    print(f'{end_time}::All paths from {start_node} to {end_node}: {count}')
    print(f'Performance: started: {start_time} finished: {end_time}')
    ...
    all_exits, successors_paths = discover_successors(grph, start_node, results=[], end_node=end_node)
